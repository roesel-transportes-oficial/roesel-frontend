...
import { Search, Plus, ArrowLeft, Save, Trash2, ChevronRight, Truck, Wrench, FileText, X, Calendar, MapPin } from 'lucide-react'
...
export default function CaminhaoPage() {
  ...
  // Aba global: caminhoes, carretas ou manutencao
  ...
  useEffect(() => {
    fetch_(); motoristasAPI.listar().then(setMotoristas).catch(() => {}); fetchFrotas(); fetchCarretas(); fetchManutencoesGlobais()
  }, [])
  ...
  async function excluirLicenca(id: string) {
    ...
    if (sel) fetchLicencas(sel.id); showMsg('Licença excluída.')
  }
    ...
    <div className="p-4 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 font-bold uppercase">Caminhões, Carretas e Manutenção</p>
        </div>
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input placeholder="Buscar placa, modelo ou frota..." value={busca} onChange={e => setBusca(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white border border-gray-100 rounded-2xl shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 text-sm" />
                </div>
                <button onClick={() => setMostraCad(!mostraCad)} className="bg-red-600 hover:bg-red-700 text-white p-3 rounded-2xl shadow-lg shadow-red-100 transition active:scale-95"><Plus size={24} /></button>
              </div>

              {mostraCad && (
                <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 space-y-4 animate-in fade-in slide-in-from-top-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filtrados.map(c => (
                  <div key={c.id} onClick={() => selecionar(c)} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md hover:border-red-100 transition cursor-pointer group relative overflow-hidden">
                    <div className={`absolute top-0 right-0 w-1.5 h-full ${c.status === 'rodando' ? 'bg-green-500' : c.status === 'manutencao' ? 'bg-yellow-500' : 'bg-gray-300'}`}></div>
                    <div className="flex items-center gap-4 mb-3">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${c.status === 'rodando' ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-400'}`}><Truck size={24} /></div>
                      <div>
                        <h3 className="font-black text-gray-900 text-lg leading-none">{c.placa}</h3>
                        <p className="text-[10px] font-bold text-gray-400 uppercase mt-1">{c.modelo} {c.ano && `· ${c.ano}`}</p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-[10px] font-bold uppercase">
                        <span className="text-gray-400">Motorista</span>
                        <span className="text-gray-700">{c.motorista_atual || '—'}</span>
                      </div>
                      <div className="flex items-center justify-between text-[10px] font-bold uppercase">
                        <span className="text-gray-400">Frota</span>
                        <span className="text-gray-700">{c.frota || '—'}</span>
                      </div>
                    </div>
                    <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between">
                      <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg ${c.status === 'rodando' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{c.status}</span>
                      <ChevronRight size={16} className="text-gray-300 group-hover:text-red-500 transition-transform group-hover:translate-x-1" />
                    </div>
                  </div>
                ))}
              </div>
                    ...
                    {licencas.length === 0 ? (<div className="text-center py-8"><FileText size={28} className="mx-auto text-gray-200 mb-2" /><p className="text-sm text-gray-400">Nenhuma licença cadastrada</p></div>) : (
                      <div className="grid grid-cols-2 gap-2">
                        {licencas.map(l => {
                          ...
                          return (
                            <div key={l.id} className={`p-3 rounded-xl border flex items-center justify-between ${vencido ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100'}`}>
                              <div>
                                <p className="text-xs font-black text-gray-800">{l.estado}</p>
                                <p className={`text-[10px] font-bold ${vencido ? 'text-red-600' : 'text-gray-400'}`}>Vence: {fmtData(l.vencimento)}</p>
                              </div>
                              <button onClick={() => excluirLicenca(l.id)} className="text-gray-300 hover:text-red-500 transition"><X size={14} /></button>
                            </div>
                          )
                        })}
                      </div>
          ...
          {!selCarreta ? (
            <>
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                  <input placeholder="Buscar carreta..." value={buscaCarreta} onChange={e => setBuscaCarreta(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white border border-gray-100 rounded-2xl shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 text-sm" />
                </div>
                <button onClick={() => setMostraCadCarreta(!mostraCadCarreta)} className="bg-red-600 hover:bg-red-700 text-white p-3 rounded-2xl shadow-lg shadow-red-100 transition active:scale-95"><Plus size={24} /></button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {carretasFiltradas.map(c => (
                  <div key={c.id} onClick={() => selecionarCarreta(c)} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition cursor-pointer group">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-gray-50 text-gray-400 flex items-center justify-center"><Truck size={24} /></div>
                      <div>
                        <h3 className="font-black text-gray-900 text-lg leading-none">{c.placa}</h3>
                        <p className="text-[10px] font-bold text-gray-400 uppercase mt-1">{c.modelo} {c.ano && `· ${c.ano}`}</p>
                      </div>
