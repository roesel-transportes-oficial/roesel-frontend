...
export default function FechamentoViagemPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('abastecimentos').select('id, data, posto, litros_combustivel, valor_combustivel, litros_arla, valor_arla')
      .eq('placa', caminhao.placa) // Busca pela placa conforme solicitado
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 bg-gray-50 min-h-screen font-sans">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 font-medium">Controle de Viagens e Financeiro</p>
        </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="p-6">
                  {!caminhao || !dataInicio || !dataFim ? (
                    <p className="text-center py-12 text-sm text-gray-400 italic">Selecione o motorista e o período para carregar os abastecimentos.</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" placeholder="Pesquisar por motorista ou placa..." value={buscaHistorico} onChange={e => setBuscaHistorico(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-red-500 transition-all text-sm font-medium" />
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">KM Inicial/Final</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {carregandoHistorico ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-gray-400 font-bold uppercase">Carregando histórico...</td></tr>
                ) : historicoFiltrado.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-gray-400 font-bold uppercase">Nenhum fechamento encontrado</td></tr>
                ) : (
                  historicoFiltrado.map(h => (
                    <tr key={h.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-xs font-bold text-gray-600">{h.km_inicial} → {h.km_final}</td>
