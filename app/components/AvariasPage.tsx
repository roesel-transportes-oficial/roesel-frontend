...
export default function AvariasPage() {
  ...
  const [sel, setSel] = useState<Avaria | null>(null)
  ...
  async function fetch_() {
    try {
      ...
      setAvarias(Array.isArray(data) ? data : [])
    } catch {}
  }
  ...
  async function excluir() {
    ...
    await fetch_(); setLoading(false); voltar(); showMsg('Avaria excluída.')
  }
  ...
  async function cadastrar() {
    ...
    setMostraCad(false); showMsg('✅ Avaria registrada!')
  }
  ...
  if (mostraCad) return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h3 className="font-bold text-gray-800 mb-4 text-lg">Nova Avaria</h3>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className={LabelClass}>Valor do Prejuízo (R$)</label>
          ...
          <div>
            ...
            <textarea value={cadDescricao} onChange={e => setCadDescricao(e.target.value)} rows={3} className={InputClass} placeholder="Descreva a avaria..." />
          </div>
          <div>
      ...
      {sel ? (
            ...
            <div className="p-5 space-y-4">
              ...
              {confirmExcluir && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-xl">
                  <p className="text-sm text-red-700 font-medium mb-3">⚠️ Excluir esta avaria?</p>
        ...
        <>
          <div className="flex items-center justify-between mb-5">
            <h1 className="text-2xl font-bold text-gray-900">Avarias</h1>
            {perm !== 'view' && (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {filtrados.length === 0 ? (
              <div className="p-10 text-center">
                <ShieldAlert size={32} className="mx-auto text-gray-200 mb-2" />
                <p className="text-sm text-gray-400">Nenhuma avaria registrada</p>
              </div>
            ) : filtrados.map(a => (
              <button key={a.id} onClick={() => selecionar(a)}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-gray-900">{a.motorista}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{a.descricao || 'Sem descrição'}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{fmtData(a.data)}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-sm font-bold text-gray-800">{(a.valor||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</p>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${a.status === 'RESOLVIDO' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    {a.status}
                  </span>
                </div>
              </button>
